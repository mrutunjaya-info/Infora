import React, { useState } from 'react';
import { X, Plus, Edit, Trash2, Search, ExternalLink, FileText } from 'lucide-react';
import { PDFResource } from '../types/syllabus';
import PDFEditor from './PDFEditor';

interface PDFManagerProps {
  subjectCode: string;
  subjectName: string;
  semesterId: number;
  isDarkMode: boolean;
  pdfs: PDFResource[];
  onClose: () => void;
  onAddPDF: (pdf: Omit<PDFResource, 'id' | 'createdAt' | 'updatedAt'>) => void;
  onUpdatePDF: (id: string, updates: Partial<PDFResource>) => void;
  onDeletePDF: (id: string) => void;
}

const PDFManager: React.FC<PDFManagerProps> = ({
  subjectCode,
  subjectName,
  semesterId,
  isDarkMode,
  pdfs,
  onClose,
  onAddPDF,
  onUpdatePDF,
  onDeletePDF,
}) => {
  const [showEditor, setShowEditor] = useState(false);
  const [editingPDF, setEditingPDF] = useState<PDFResource | undefined>();
  const [searchTerm, setSearchTerm] = useState('');

  const filteredPDFs = pdfs.filter(pdf =>
    pdf.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    pdf.url.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleEditPDF = (pdf: PDFResource) => {
    setEditingPDF(pdf);
    setShowEditor(true);
  };

  const handleNewPDF = () => {
    setEditingPDF(undefined);
    setShowEditor(true);
  };

  const handleCloseEditor = () => {
    setShowEditor(false);
    setEditingPDF(undefined);
  };

  const handleOpenPDF = (url: string) => {
    // Open PDF in Google Drive viewer
    const driveViewerUrl = `https://drive.google.com/viewerng/viewer?embedded=true&url=${encodeURIComponent(url)}`;
    window.open(driveViewerUrl, '_blank');
  };

  const formatDate = (date: Date) => {
    const dateObj = date instanceof Date ? date : new Date(date);
    return dateObj.toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
    });
  };

  return (
    <>
      <div className={`fixed inset-0 ${isDarkMode ? 'bg-black' : 'bg-black'} bg-opacity-50 z-40 flex items-center justify-center p-1`}>
        <div className={`${isDarkMode ? 'bg-black border border-white' : 'bg-white'} rounded-lg shadow-xl w-full max-w-4xl h-[90vh] flex flex-col`}>
          <div className="flex items-center justify-between p-4 border-b">
            <div>
              <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>PDF Resources</h2>
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{subjectCode}: {subjectName}</p>
            </div>
            <div className="flex items-center space-x-2">
              <button
                onClick={handleNewPDF}
                className="flex items-center px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors"
              >
                <Plus className="w-4 h-4 mr-2" />
                Add PDF
              </button>
              <button
                onClick={onClose}
                className="p-2 text-gray-500 hover:text-gray-700 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>

          <div className="p-4 border-b">
            <div className="relative">
              <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
              <input
                type="text"
                placeholder="Search PDFs..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className={`w-full pl-10 pr-4 py-2 border ${isDarkMode ? 'border-white bg-black text-white focus:ring-white' : 'border-gray-300 bg-white text-gray-900 focus:ring-red-500'} rounded-lg focus:outline-none focus:ring-2 focus:border-transparent`}
              />
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-4">
            {filteredPDFs.length === 0 ? (
              <div className={`text-center py-12`}>
                <FileText className={`w-16 h-16 ${isDarkMode ? 'text-white' : 'text-gray-400'} mx-auto mb-4`} />
                <p className={`${isDarkMode ? 'text-white' : 'text-gray-500'} mb-4`}>
                  {searchTerm ? 'No PDFs found matching your search.' : 'No PDF resources yet for this subject.'}
                </p>
                {!searchTerm && (
                  <button
                    onClick={handleNewPDF}
                    className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors"
                  >
                    Add your first PDF
                  </button>
                )}
              </div>
            ) : (
              <div className="space-y-4">
                {filteredPDFs.map((pdf) => (
                  <div key={pdf.id} className={`${isDarkMode ? 'bg-black border-white' : 'bg-gray-50 border-gray-200'} rounded-lg p-4 border`}>
                    <div className="flex items-start justify-between mb-2">
                      <h3 className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{pdf.title}</h3>
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => handleOpenPDF(pdf.url)}
                          className={`p-1 ${isDarkMode ? 'text-white hover:text-gray-300' : 'text-gray-500 hover:text-red-600'} transition-colors`}
                          title="Open PDF"
                        >
                          <ExternalLink className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleEditPDF(pdf)}
                          className={`p-1 ${isDarkMode ? 'text-white hover:text-gray-300' : 'text-gray-500 hover:text-blue-600'} transition-colors`}
                        >
                          <Edit className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => onDeletePDF(pdf.id)}
                          className={`p-1 ${isDarkMode ? 'text-white hover:text-gray-300' : 'text-gray-500 hover:text-red-600'} transition-colors`}
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
                    <p className={`${isDarkMode ? 'text-white' : 'text-gray-600'} text-sm mb-3 break-all`}>
                      {pdf.url}
                    </p>
                    <div className={`flex items-center justify-between text-xs ${isDarkMode ? 'text-white' : 'text-gray-500'}`}>
                      <span>Added: {formatDate(pdf.createdAt)}</span>
                      {new Date(pdf.updatedAt).getTime() !== new Date(pdf.createdAt).getTime() && (
                        <span>Updated: {formatDate(pdf.updatedAt)}</span>
                      )}
                    </div>
                  </div>
                ))}
              </div>
            )}
          </div>
        </div>
      </div>

      {showEditor && (
        <PDFEditor
          pdf={editingPDF}
          isDarkMode={isDarkMode}
          subjectCode={subjectCode}
          semesterId={semesterId}
          onSave={onAddPDF}
          onUpdate={onUpdatePDF}
          onClose={handleCloseEditor}
        />
      )}
    </>
  );
};

export default PDFManager;